...
const FilaProfesorExterno = ({ profesor, onDelete, onAutorizar }) => {
    ...
    useEffect(() => {
        const cargarNiveles = async () => {
            try {
                const response = await axios.get('https://backend-sistema-apoyo-production.up.railway.app/API/Nivel/Listar Niveles');
    ...
    const handleAutorizarProfesor = async () => {
        if (window.confirm(`¿Estás seguro de que deseas enviar para autorización al profesor ${profesor.nombreCompleto}?`)) {
            try {
                ...
                await axios.post('https://backend-sistema-apoyo-production.up.railway.app/API/AdministradorProfesor/CrearProfesor', profesorData);
                await axios.delete(`https://backend-sistema-apoyo-production.up.railway.app/api/Bolsatrabajo/EliminarBolsa?id=${profesor.idbolsa}`);
