...
const TeacherCard = ({ teacher }) => {
  ...
  // Process the image URL to ensure it's complete
  const getImageUrl = (imageUrl) => {
    ...
    return `https://backend-sistema-apoyo-production.up.railway.app${imageUrl.startsWith('/') ? '' : '/'}${imageUrl}`;
  };
  ...
  return (
    <div className="bg-white p-4 rounded-xl shadow-lg flex flex-col items-center">
      <div className="w-32 h-32 rounded-full overflow-hidden mx-auto border-4 border-gray-200">
        <img 
          src={imageError ? placeholderImage : getImageUrl(teacher.image)}
          alt={teacher.name} 
      ...
      try {
        ...
        reviewsList.forEach((review) => {
          ...
          if (!teachersMap[profesorId]) {
            ...
            // Log image URL to debug
            const imageUrl = getPropertySafely(profesorUser, ['fotoRuta', 'foto', 'imagen', 'image']);
            console.log(`Teacher ${profesorId} image URL:`, imageUrl);
        ...
        <div className="px-6 py-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {filteredTeachers.length > 0 ? (
              ...
              <p className="col-span-3 text-center text-gray-500">No se encontraron profesores para el nivel seleccionado.</p>
            )}
          </div>
        </div>
      </div>
