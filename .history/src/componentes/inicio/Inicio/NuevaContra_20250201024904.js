...
const ResetPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Cuerpo de la solicitud:", JSON.stringify({ Token: token, NuevaContraseña: newPassword }));
      ...
      const response = await fetch('http://localhost:5228/API/Usuario/reestablecer-contrasena', {
        ...
        body: JSON.stringify({ Token: token, NuevaContraseña: newPassword }), // Ajusta los nombres de los campos
      });
      ...
      setTimeout(() => {
        navigate('/login'); // Ajusta la ruta al login si es necesario
      }, 2000); // Redirige después de 2 segundos, para que el mensaje se vea antes de la redirección
