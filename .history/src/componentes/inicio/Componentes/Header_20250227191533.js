...
const Header = () => {
  ...
  // If user is logged in but on the home page, show the non-logged-in header
  if (!user || (user && isHomePage)) {
    return (
      <div className="flex flex-col w-full">
        <header className="flex items-center justify-between p-6" style={{ backgroundColor: "#00A89F" }}>
          ...
          <nav className="hidden md:flex gap-6">
            {user && isHomePage ? (
              // If user is logged in and on home page, show user info instead of login/register
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-teal-600 p-4">
            <div className="flex flex-col gap-4">
              ...
              {user && isHomePage ? (
                // If user is logged in and on home page, show user options
                <>
                  <button
                    onClick={() => {
                      goToChat()
