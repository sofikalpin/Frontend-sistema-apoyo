...
const Header = () => {
  ...
  }, [location.pathname]) // Corrected dependency to close mobile menu when route changes

  // Check if we're on the home page
  const isHomePage = location.pathname === "/"

  // Always show the non-logged-in header on the home page, regardless of login status
  if (!user || isHomePage) {
    return (
      <div className="flex flex-col w-full">
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-teal-600 p-4 flex flex-col items-center">
            <button
              onClick={() => {
                navigate("/inicioprofesor")
                toggleMobileMenu()
              }}
              className="text-white font-bold hover:text-yellow-400 text-xl py-2 w-full text-center"
            >
              Profesores
            </button>
            <div className="w-full relative">
              <button
                ...
                className="text-white font-bold hover:text-yellow-400 text-xl py-2 w-full text-center flex items-center justify-center"
              >
                Programas
                <ChevronDown className={`w-5 h-5 ml-2 transition-transform ${isProgramsOpen ? "rotate-180" : ""}`} />
              </button>
              {isProgramsOpen && <ProgramDropdown isOpen={true} isMobile={true} onClose={() => setIsProgramsOpen(false)} />}
            </div>
            <button
              onClick={() => {
  ...
  // Regular logged-in header for all other routes
  return (
    <div className="flex flex-col w-full">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl">
            ...
            <div className="flex justify-end gap-4">
              <button onClick={closeLogoutModal} className="px-4 py-2 text-gray-600 hover:text-gray-800">
                Cancelar
              </button>
