...
export default function InicioProfesor() {
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-8">
      ...
      <div className="bg-gray-200 p-4 rounded-xl shadow-lg max-w-7xl fixed right-5 top-1/4 flex justify-end">
      <select
  value={selectedLevel}
  onChange={handleLevelChange}
  className="px-12 py-6 text-3xl border rounded bg-green-500 hover:bg-green-600 w-full max-w-xs"
  aria-label="Filtrar por nivel"
  style={{ color: "black" }} // Estilo adicional para garantizar el color negro
>
  ...
  {filteredTeachers.length > 0 ? (
    <div className="grid grid-cols-3 sm:grid-cols-2 lg:grid-cols-3 gap-6 ">
      {filteredTeachers.map((teacher) => (
        <div
          key={teacher.name}
          className="bg-gray-200 p-4 rounded-xl shadow-lg w-full h-80 flex flex-col items-center justify-between"
        >
          <div>
            <div className="mb-4 ">
              <img
          ...
          <div className="mt-4">
            <button
              className="text-blue-500 text-xl focus:outline-none mg-5"
