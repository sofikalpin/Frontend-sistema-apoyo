  ...
  const cargarFotoExistente = async () => {
    try {
      const response = await axios.get(
        `https://backend-sistema-apoyo-production.up.railway.app/API/Usuario/ObtenerFoto/${user.correo}`,
        {
          responseType: 'blob'
        }
      );
  ...
  const handleUpdatePhoto = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://backend-sistema-apoyo-production.up.railway.app/API/Usuario/ActualizarFoto',
        {
          correo: user.correo,
          foto: photo
        },
