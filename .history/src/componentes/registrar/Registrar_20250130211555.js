...
const handleSubmit = (e) => {
  e.preventDefault();
  if (validateStep()) {
    // Asignamos el rol dependiendo de la selección
    const role = formData.userType === "profesor" ? 1 : 2; // 1 para profesor, 2 para alumno

    const usuario = {
      nombrecompleto: formData.name,
      correo: formData.email,
      contraseñaHash: formData.password, // Aquí debes hacer el hash de la contraseña antes de enviarlo
      fecharegistro: new Date().toISOString(),
      idnivel: formData.nivel, // El nivel seleccionado por el usuario
      idrol: role, // El id del rol (1 o 2)
      autProf: formData.userType === "profesor", // Si es profesor, true
      tokenRecuperacion: "string", // O genera un token aquí si es necesario
      tokenExpiracion: new Date(new Date().getTime() + 3600 * 1000).toISOString(), // Un token que expire en una hora
    };

    console.log("Datos del formulario:", usuario);
    // Enviar los datos al backend
    fetch("http://localhost:5228/API/Usuario/GuardarUsuario", {
      method: "POST",
      headers: {
