...
const handleLogin = async ({ email, password }) => {
  try {
    const response = await fetch('http://localhost:5000/api/Usuario/IniciarSesion', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      saveUserSession(response.token);
      navigate('/dashboard');
    } catch (error) {
      console.error('Error en submit:', error);
      setErrors({
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="text-center mt-4">
                <button
                  ...
                  className="flex items-center justify-center w-full p-3 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50"
                >
