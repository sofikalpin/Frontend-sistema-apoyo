...
// También actualicemos el handleSubmit para manejar mejor los errores
const handleSubmit = async (e) => {
  e.preventDefault();
  if (!validateForm()) {
    return;
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.token) {
        saveUserSession(response.token, formData.rememberMe);
        navigate('/dashboard'); // Asegúrate de tener configurada esta ruta
      } else {
        throw new Error('No se recibió el token de autenticación');
      }
    } catch (error) {
      setErrors({
        submit: 'Credenciales inválidas. Por favor verifica tu correo y contraseña.'
      });
    } finally {
      setIsLoading(false);
    }
