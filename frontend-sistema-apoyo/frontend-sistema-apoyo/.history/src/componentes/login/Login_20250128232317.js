...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    setMensaje(data.msg); // Muestra el mensaje de éxito
  } catch (error) {
    setMensaje(error.message); // Muestra el mensaje de error
  } finally {
    setLoading(false);
  }
};
          ...
          <div className="w-full max-w-md mx-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <p className="mt-4 text-sm text-gray-600">
                ¿No tienes cuenta? {' '}
                <Link to="/registrarse" className="text-blue-600 hover:text-blue-800">Regístrate gratis</Link>
              </p>
            </form>
          </div>
        </div>
