...
const NuevaConsulta = ({ onBack }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Simula un retraso para enviar los datos (puedes reemplazarlo por una llamada real al backend)
    setTimeout(() => {
      console.log({ titulo, categoria, consulta });
      setLoading(false);
      alert('Consulta publicada con éxito');
      // Aquí podrías redirigir o realizar alguna acción después de la publicación.
    }, 2000);
  };
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-3xl space-y-6">
        ...
        <div className="flex flex-col gap-2">
          ...
          <textarea 
            ...
            aria-describedby="consulta-help"
          />
        </div>
        ...
        <div className="flex gap-4">
          ...
          <button 
            type="submit" 
            className={`w-full ${loading ? 'bg-gray-400' : 'bg-teal-600 hover:bg-teal-700'} text-white px-6 py-4 rounded-lg font-semibold flex items-center justify-center gap-3 shadow-md transition-all duration-300`}
            disabled={loading}
            aria-label="Publicar consulta"
          >
            {loading ? (
