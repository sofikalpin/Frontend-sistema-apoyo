...
export const Registrar = () => {
  ...
  // Guardamos únicamente los correos existentes
  ...
  // Al montar el componente, obtenemos la lista de usuarios y extraemos solo los correos
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // Extraemos solo los correos y los guardamos en minúsculas para comparar fácilmente
        const emails = usersArray.map((user) => user.correo.toLowerCase());
  ...
  const validateEmail = async () => {
    ...
    try {
      ...
      const emailExists = existingEmails.includes(emailEntered);
      if (emailExists) {
  ...
  const validateStep = async () => {
    ...
    if (step === 3) {
      // Para el paso 3, usamos la función validateEmail
