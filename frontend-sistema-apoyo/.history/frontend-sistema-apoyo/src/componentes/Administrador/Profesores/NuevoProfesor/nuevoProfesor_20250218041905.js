...
export const NuevoProfesor = () => {
  ...
  const handleRegistrar = async (e) => {
    ...
    try {
      ...
      if (professorResponse.data.status) {
        // Si la creación fue exitosa, subimos el CV
        const cvFormData = new FormData();
        cvFormData.append("archivo", cv);

        const cvResponse = await axios.post(
          `http://localhost:5228/API/Usuario/SubirCV?idUsuario=${professorResponse.data.value.idusuario}`,
          cvFormData,
          {
            headers: {
        ...
        if (cvResponse.data.status) {
          window.alert("Profesor creado con éxito y CV subido correctamente.");
          navigate(-1);
        } else {
          alert("Profesor creado pero hubo un problema al subir el CV: " + cvResponse.data.msg);
        }
      } else {
        alert(professorResponse.data.msg || "No se pudo crear el profesor.");
      }
    } catch (error) {
