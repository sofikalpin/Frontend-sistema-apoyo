...
import Footer from '../Administrador/FooterAdministrador';
        ...
        <div className="container mx-auto flex flex-col items-center">
            <div className="flex space-x-6 mb-4">
                {socialIcons.map((icon) => (
                    <span key={icon.name} className={`cursor-pointer ${icon.color}`}>
                        {icon.name}
                    </span>
                ))}
            </div>
            <p className="text-sm">&copy; {new Date().getFullYear()} Edu-Match. Todos los derechos reservados.</p>
        </div>
    </footer>
);
