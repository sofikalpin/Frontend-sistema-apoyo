...
const CargarProfesorExterno = () => {
    ...
    // Updated delete function with different approaches
    const handleDeleteProfesor = async (idbolsa) => {
        try {
            ...
            if (window.confirm("¿Estás seguro de que deseas rechazar este profesor?")) {
                // Approach 1: Using axios config object
                await axios({
                    method: 'delete',
                    url: 'http://localhost:5228/api/Bolsatrabajo/EliminarBolsa',
                    params: { id: idbolsa }
                });

                // If the above doesn't work, try Approach 2: Using data in request body
                /* 
                await axios.delete('http://localhost:5228/api/Bolsatrabajo/EliminarBolsa', {
                    data: { id: idbolsa }
                });
                */

                // If the above doesn't work, try Approach 3: Direct URL with template literal
                /*
    ...
    const handleAutorizarProfesor = async (idbolsa) => {
        try {
            ...
            setError("Ocurrió un error al autorizar el profesor.");
            setTimeout(() => setError(""), 3000);
        }
    };
