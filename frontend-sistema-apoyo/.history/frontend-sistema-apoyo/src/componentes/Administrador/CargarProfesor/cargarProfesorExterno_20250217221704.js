    ...
    const handleDeleteProfesor = async (idbolsa) => {
        try {
            ...
            if (window.confirm("¿Estás seguro de que deseas rechazar este profesor?")) {
                const response = await axios.delete('http://localhost:5228/api/Bolsatrabajo/EliminarBolsa', {
                    params: {
                        id: idbolsa
                    }
                });
            ...
            if (error.response) {
                ...
                setError(`Error: No se pudo rechazar al profesor. Código: ${error.response.status}`);
