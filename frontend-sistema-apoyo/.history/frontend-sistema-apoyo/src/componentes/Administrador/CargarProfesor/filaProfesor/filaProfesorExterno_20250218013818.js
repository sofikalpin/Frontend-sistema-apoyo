    ...
    const handleAutorizarProfesor = async () => {
        if (window.confirm(`¿Estás seguro de que deseas enviar para autorización al profesor ${profesor.nombreCompleto}?`)) {
            try {
                ...
                // Obtener el idnivel correcto basado en la descripción del nivel
                const nivelId = getNivelIdFromDescripcion(profesor.nivel);
                
                if (!nivelId) {
                    throw new Error('No se pudo encontrar el ID del nivel correspondiente');
                }
                ...
                const profesorData = {
                    ...
                    idnivel: nivelId, // Usando el ID del nivel mapeado
                    idrol: 2,
                    autProf: true, 
