...
const FilaProfesorExterno = ({ profesor, onDelete, onAutorizar }) => {
    ...
    const handleAutorizarProfesor = async () => {
        if (window.confirm(`¿Estás seguro de que deseas enviar para autorización al profesor ${profesor.nombreCompleto}?`)) {
            try {
                // Crear el objeto con los datos del profesor para la tabla de autorización
                const profesorData = {
                ...
                await axios.post('http://localhost:5228/API/AdministradorProfesor/CrearProfesorPendiente', profesorData);
    ...
    return (
        <tr className="border-b hover:bg-gray-100">
            ...
            <td className="px-4 py-3 text-center">{profesor.nivel}</td>
