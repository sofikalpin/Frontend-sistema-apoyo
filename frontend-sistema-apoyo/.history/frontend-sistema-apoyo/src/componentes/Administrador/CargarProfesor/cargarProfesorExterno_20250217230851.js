...
const CargarProfesorExterno = () => {
    ...
    const handleDeleteProfesor = async (idusuario) => {
        try {
            console.log("Eliminando profesor con idbolsa:", idusuario); // Verifica que se pase el idbolsa correcto
            if (!idusuario) {
                throw new Error("ID de bolsa no válido");
            }

            if (window.confirm("¿Estás seguro de que deseas rechazar este profesor?")) {
                await axios.delete(`http://localhost:5228/api/Bolsatrabajo/EliminarBolsa/${idusuario}`);
