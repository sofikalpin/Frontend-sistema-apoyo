...
const CargarProfesorExterno = () => {
    ...
    const handleAutorizarProfesor = async (key) => {
        try {
            console.log("Autorizando profesor con idbolsa:", key); // Verifica que se pase el idbolsa correcto
            if (!idbolsa) {
                throw new Error("ID de bolsa no válido");
            }

            if (window.confirm("¿Estás seguro de que deseas autorizar este profesor?")) {
                await axios.put(`http://localhost:5228/API/AdministradorProfesor/AutorizarProfesor/${key}`);
