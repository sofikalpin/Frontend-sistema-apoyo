...
const CargarProfesorExterno = () => {
    ...
    const handleDeleteProfesor = (id) => {
        if (window.confirm("¿Estás seguro de que deseas rechazar este profesor?")) {
            axios.delete(`http://localhost:5228/api/Bolsatrabajo/EliminarBolsa?id=${id}`)
                .then(() => {
            ...
            try {
                ...
                if (Array.isArray(response.data)) {
                    const datosLimpios = response.data.map((profesor, index) => ({
                        key: profesor.idbolsa ?? `${profesor.idusuario}-${index}`,
                        ...
                        // Adding nameDisplay for the table to use
                        nameDisplay: (
