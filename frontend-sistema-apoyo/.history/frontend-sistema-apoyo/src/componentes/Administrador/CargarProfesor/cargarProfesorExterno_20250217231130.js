...
const CargarProfesorExterno = () => {
    ...
    const handleDeleteProfesor = async (bolsa) => {
        try {
            console.log("Eliminando profesor con idbolsa:",bolsa); // Verifica que se pase el idbolsa correcto
            if (!bolsa) {
                throw new Error("ID de bolsa no válido");
            }

            if (window.confirm("¿Estás seguro de que deseas rechazar este profesor?")) {
                await axios.delete(`http://localhost:5228/api/Bolsatrabajo/EliminarBolsa/${bolsa}`);
                
                setProfesoresIngles((prevProfesores) => 
                    prevProfesores.filter((profesor) => profesor.bolsa !== bolsa)
                );
