...
const CargarProfesorExterno = () => {
    ...
    const handleAutorizarProfesor = (idusuario) => {
        if (window.confirm("¿Estás seguro de que deseas autorizar este profesor?")) {
            axios.put(`https://backend-sistema-apoyo-production.up.railway.app/API/AdministradorProfesor/AutorizarProfesor?id=${idusuario}`)
                .then(() => {
                    setProfesoresIngles((prevProfesores) => 
    ...
    useEffect(() => {
        const fetchProfesores = async () => {
            setLoading(true);
            try {
                const response = await axios.get("https://backend-sistema-apoyo-production.up.railway.app/api/Bolsatrabajo/ingles");
