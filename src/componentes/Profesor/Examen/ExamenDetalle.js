...
const ExamenDetalle = () => {
  ...
  useEffect(() => {
    const encontrarExamen = async () => {
      try {
        setLoading(true);
        const respuesta = await axios.get(`hhttps://backend-sistema-apoyo-production.up.railway.app/api/examenes/ExamenID?id=${idexamen}`);
  ...
  const handleDelete = async () => {
    ...
    if (window.confirm("¿Está seguro que desea eliminar este examen?")) {
      try {
        setLoading(true);
        const respuesta = await axios.delete(`https://backend-sistema-apoyo-production.up.railway.app/api/ProfeExamen/EliminarExamen?id=${idexamen}`);
        if (respuesta.data.status) {
